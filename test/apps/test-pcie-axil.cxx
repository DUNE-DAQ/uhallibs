...
#define get_syspath_bar_mmap(s, bus, dev, func, bar)                                                                   \
  snprintf(s, sizeof(s), "/sys/bus/pci/devices/0000:%02x:%02x.%x/resource%u", bus, dev, func, bar)

static uint32_t*
mmap_bar(char* fname, size_t len, int prot)
{
  int fd;
  void* bar;
...
static int32_t
reg_write(uint64_t waddr, uint32_t wval)
{
  uint32_t* bar;
  char fname[256];

  std::cout << "0x" << std::hex << std::setw(8) << std::setfill('0') << waddr << "   " << std::setw(8)
            << std::setfill('0') << wval << std::endl;
...
main()
{
  ...
  for (uint64_t i(0); i < 8; ++i) {
    ...
    // rd_buf[i] = rval;
    std::cout << "0x" << std::hex << std::setw(8) << std::setfill('0') << read_base + i << "   " << std::setw(8)
              << std::setfill('0') << rval << std::endl;
  }
