...
//-----------------------------------------------------------------------------
regmap_register_t*
Flx::Card::find_reg(const std::string& aName)
{
...
//-----------------------------------------------------------------------------
void
Flx::Card::read(const uint32_t aAddr, const uint32_t aNrWords, std::vector<uint32_t>& aValues)
{
...
//-----------------------------------------------------------------------------
void
Flx::Card::write(const uint32_t aAddr, const std::vector<std::pair<const uint8_t*, size_t>>& aData)
{
...
// Flx Transport
std::string
Flx::getSharedMemName(const std::string& aPath)
{
  ...
  , mAsynchronousException(NULL)
{
  ...
  for (uhal::NameValuePairVectorType::const_iterator lIt = aUri.mArguments.begin(); lIt != aUri.mArguments.end();
       lIt++) {
    if (lIt->first == "sleep") {
      ...
      log(uhal::Notice(),
          "flx client with URI ",
          uhal::Quote(uri()),
          " : Inter-poll-/-interrupt sleep duration set to ",
          std::stoul(lIt->second),
    ...
    //   log (uhal::Notice(), "flx client with URI ", uhal::Quote (uri()), " : Address offset set to ",
    //   uhal::Integer(lOffset, IntFmt<hex>()));
    // }
    else {
...
void
Flx::implementDispatch(std::shared_ptr<uhal::Buffers> aBuffers)
{
...
void
Flx::dispatchExceptionHandler()
{
  // FIXME: Adapt to PCIe implementation
  // log(uhal::Notice(), "flx client ", uhal::Quote(id()), " (URI: ", uhal::Quote(uri()), ") : closing device files
...
void
Flx::connect(IPCScopedLock_t& aGuard)
{
  ...
  log(uhal::Debug(),
      ...
      "): ",
      PacketFmt((const uint8_t*)lValues.data(), 4 * lValues.size()));
  aGuard.unlock();
  ...
  if (lValues.at(1) > 0xFFFF) {
    ...
    log(lExc,
        "Invalid page size, ",
        uhal::Integer(lValues.at(1)),
        ", reported in device file ",
        uhal::Quote(mDeviceFile.getPath()));
    throw lExc;
  }
  ...
  if (mIndexNextPage >= mNumberOfPages) {
    ...
    log(lExc,
        "Next page index, ",
        uhal::Integer(mIndexNextPage),
        ", reported in device file ",
        uhal::Quote(mDeviceFile.getPath()),
        " is inconsistent with number of pages, ",
        uhal::Integer(mNumberOfPages));
    throw lExc;
  }

  mConnected = true;
  log(uhal::Info(),
      "flx client connected to device at ",
      uhal::Quote(mDeviceFile.getPath()),
      "; FPGA has ",
      uhal::Integer(mNumberOfPages),
      " pages, each of size ",
      uhal::Integer(mPageSize),
      " words, index ",
      uhal::Integer(mIndexNextPage),
...
void
Flx::write(const std::shared_ptr<uhal::Buffers>& aBuffers)
{
  ...
  log(uhal::Info(),
      ...
      uhal::Quote(uri()),
      ") : writing ",
      uhal::Integer(aBuffers->sendCounter() / 4),
      "-word packet to page ",
      uhal::Integer(mIndexNextPage),
      " in ",
      uhal::Quote(mDeviceFile.getPath()));
  ...
  log(uhal::Debug(),
      "Wrote ",
      uhal::Integer((aBuffers->sendCounter() / 4) + 1),
      " 32-bit words at address ",
      uhal::Integer(mIndexNextPage * 4 * mPageSize),
      " ... ",
...
Flx::read()
{
  ...
  if (mReadReplyPageCount == mPublishedReplyPageCount) {
    ...
    while (true) {
      ...
      // log (uhal::Info(), "Read status info from addr 0 (", uhal::Integer(lValues.at(0)), ", ",
      // uhal::Integer(lValues.at(1)), ", ", uhal::Integer(lValues.at(2)), ", ", uhal::Integer(lValues.at(3)), "): ",
      // PacketFmt((const uint8_t*)lValues.data(), 4 * lValues.size()));
      log(uhal::Debug(),
          "Read status info from addr 0 (",
          uhal::Integer(lValues.at(0)),
      ...
      if (SteadyClock_t::now() - lStartTime > std::chrono::microseconds(getBoostTimeoutPeriod().total_microseconds())) {
        ...
        log(lExc,
            "Next page (index ",
            uhal::Integer(lPageIndexToRead),
            " count ",
            uhal::Integer(mPublishedReplyPageCount + 1),
            ") of flx device '" + mDeviceFile.getPath() + "' is not ready after timeout period");
        throw lExc;
      }
      ...
      log(uhal::Debug(),
          ...
          uhal::Integer(lPageIndexToRead),
          " = count ",
          uhal::Integer(mReadReplyPageCount + 1),
          "; published page count is ",
          uhal::Integer(lHwPublishedPageCount),
          "; sleeping for ",
  ...
  log(uhal::Debug(),
      "Read ",
      uhal::Integer(lNrWordsToRead),
      " 32-bit words from address ",
      uhal::Integer(4 + lPageIndexToRead * 4 * mPageSize),
      " ... ",
      PacketFmt((const uint8_t*)lPageContents.data(), 4 * lPageContents.size()));
  ...
  if (lNrWordsInPacket != (lBuffers->replyCounter() >> 2))
    log(uhal::Warning(),
        ...
        uhal::Integer(lBuffers->replyCounter() >> 2),
        " words, but it actually contains ",
        uhal::Integer(lNrWordsInPacket),
        " words");

  size_t lNrBytesCopied = 0;
  for (std::deque<std::pair<uint8_t*, uint32_t>>::const_iterator lIt = lReplyBuffers.begin();
       lIt != lReplyBuffers.end();
       ++lIt) {
  ...
  try {
    if (uhal::exception::exception* lExc =
          ClientInterface::validate(lBuffers)) // Control of the pointer has been passed back to the client interface
    {
      mAsynchronousException = lExc;
    }
  } catch (uhal::exception::exception& aExc) {
    mAsynchronousException = new uhal::exception::ValidationError();
    log(*mAsynchronousException,
        "Exception caught during reply validation for flx device with URI ",
        uhal::Quote(this->uri()),
        "; what returned: ",
        uhal::Quote(aExc.what()));
  }
