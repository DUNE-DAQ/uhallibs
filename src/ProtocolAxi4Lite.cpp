...
const std::string&
Axi4Lite::MappedFile::getPath() const
{
  return mPath;
}

void
Axi4Lite::MappedFile::setPath(const std::string& aPath)
{
  mPath = aPath;
}
void
Axi4Lite::MappedFile::setLength(size_t aLength)
{
  mLength = aLength;
}

void
Axi4Lite::MappedFile::open()
{
  if (mBar != nullptr)
    return;
...
void
Axi4Lite::MappedFile::createBuffer(const size_t aNrBytes)
{
  if (mBuffer != NULL) {
  ...
  if (mBuffer == NULL) {
    ...
    log(lExc, "Failed to allocate ", Integer(aNrBytes + 4096), " bytes in Axi4Lite::MappedFile::createBuffer");
    throw lExc;
  }
...
void
Axi4Lite::MappedFile::read(const uint32_t aAddr, const uint32_t aNrWords, std::vector<uint32_t>& aValues)
{
  if (mBar == nullptr)
    open();
...
void
Axi4Lite::MappedFile::write(const uint32_t aAddr, const std::vector<uint32_t>& aValues)
{
  write(aAddr, reinterpret_cast<const uint8_t*>(aValues.data()), 4 * aValues.size());
}

void
Axi4Lite::MappedFile::write(const uint32_t aAddr, const uint8_t* const aPtr, const size_t aNrBytes)
{
  if (mBar == nullptr)
    open();
...
void
Axi4Lite::MappedFile::write(const uint32_t aAddr, const std::vector<std::pair<const uint8_t*, size_t>>& aData)
{
  if (mBar == nullptr)
    open();
...
bool
Axi4Lite::MappedFile::haveLock() const
{
  return mLocked;
}

void
Axi4Lite::MappedFile::lock()
{
...
// Axi4Lite Transport
std::string
Axi4Lite::getSharedMemName(const std::string& aPath)
{
...
std::string
Axi4Lite::getDevicePath(const uhal::URI& aUri)
{

  std::string lPath = aUri.mHostname;

  auto it = std::find_if(aUri.mArguments.begin(),
                         aUri.mArguments.end(),
...
void
Axi4Lite::implementDispatch(std::shared_ptr<uhal::Buffers> aBuffers)
{
...
void
Axi4Lite::dispatchExceptionHandler()
{
  log(uhal::Notice(),
      "Axi4Lite client ",
      uhal::Quote(id()),
...
void
Axi4Lite::connect(IPCScopedLock_t& aGuard)
{
  ...
  log(uhal::Debug(),
      "Axi4Lite client is opening device file ",
      uhal::Quote(mMappedFile.getPath()),
      " (device-to-client)");
  ...
  log(uhal::Info(),
      "Axi4Lite client connected to device at ",
      uhal::Quote(mMappedFile.getPath()),
      ", FPGA has ",
...
void
Axi4Lite::write(const std::shared_ptr<uhal::Buffers>& aBuffers)
{
  ...
  log(uhal::Debug(),
      "Wrote ",
      Integer((aBuffers->sendCounter() / 4) + 1),
      " 32-bit words at address ",
      Integer(mIndexNextPage * mPageSize),
      " ... ",
...
Axi4Lite::read()
{
    ...
    while (true) {
      ...
      if (SteadyClock_t::now() - lStartTime > std::chrono::microseconds(getBoostTimeoutPeriod().total_microseconds())) {
        ...
        log(lExc,
            ...
            " count ",
            Integer(mPublishedReplyPageCount + 1),
            ") of Axi4Lite device '" + mMappedFile.getPath() + "' is not ready after timeout period");
        throw lExc;
      }
  ...
  try {
    ...
    log(lExc2,
        "Exception caught during reply validation for Axi4Lite device with URI ",
        uhal::Quote(this->uri()),
        "; what returned: ",
